...
(function () {
    ...
    Scrunch = (function () {
        ...
        var Scrunch = function (startFile, verbose) {
            if (!startFile) {
        ...
        Scrunch.prototype.parse = function (filepath) {
            ...
            for (i = 0; i < dependencies.length; i += 1) {
                ...
                dependencyPath = path.join(folder, dependency);
                dependencyPath = utils.getExtension(dependencyPath).fullpath;

                if (this.verbose) {
                    console.log(filepath + ' --> ' + dependency);
                }
        ...
        Scrunch.prototype.compile = function (options) {
            ...
            for (path in this.cache) {
                if (this.cache.hasOwnProperty(path)) {
                    ...
                    // Fix for javascript
                    if (file.type === 'js') {
                        this.output += '      `' + uglify.minify(file.contents.replace(/\`/, '\\`'), {fromString: true}).code + '`';
                    } else {
